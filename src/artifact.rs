//! artifact.rs — Artifact representation and pure-std SHA-256 hasher.
//! Zero crates.io dependencies. FIPS 180-4 compliant.
...
pub fn load_artifact(path: &Path) -> Result<Artifact, ArtifactError> {
    let bytes = fs::read(path)?;
    let name = path.file_name().and_then(|n| n.to_str()).unwrap_or("artifact");
    Ok(Artifact::from_bytes(name, &bytes))
}
...
// Pure std:: SHA-256 implementation
const K: [u32; 64] = [
