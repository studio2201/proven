//! doctor.rs — System & Environment Diagnostics for proven.
...
fn check_binary() -> DoctorCheck {
    let exe = env::current_exe().unwrap_or_else(|_| PathBuf::from("proven"));
    if !exe.exists() {
        return DoctorCheck {
            name: "binary_integrity".into(),
            status: "error".into(),
            detail: format!("Binary not found at {}", exe.display()),
            remediation: Some("Reinstall proven via install.sh".into()),
        };
    }
